...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Call Dextro with fAggressive=true to
    // get as much data as possible.
    // Rewrite dextrod data to wallet.dat
    ...
    std::vector<CDBEnv::KeyValPair> dextrodData;
    bool allOK = dbenv.Dextro(newFilename, true, dextrodData);
    if (dextrodData.empty()) {
        LogPrintf("Dextro(aggressive) found no records in %s.\n", newFilename);
        return false;
    }
    LogPrintf("Dextro(aggressive) found %u records\n", dextrodData.size());
    ...
    BOOST_FOREACH (CDBEnv::KeyValPair& row, dextrodData) {
        if (fOnlyKeys) {
